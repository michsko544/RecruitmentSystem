...
function getRepliesData($user){
    ...
    $query_id = "SELECT c.id_conv FROM users u join messages m on u.id_user=m.id_user join conv c on m.id_conv=c.id_conv join users u1 on u1.id_user=m.id_sender where u.id_user = '{$user}' group by c.id_conv order by m.time";
    $query_to = "SELECT c.topic FROM users u join messages m on u.id_user=m.id_user join conv c on m.id_conv=c.id_conv join users u1 on u1.id_user=m.id_sender where u.id_user = '{$user}' group by c.id_conv order by m.time";
    $query_ti = "SELECT MAX(m.time) FROM users u join messages m on u.id_user=m.id_user join conv c on m.id_conv=c.id_conv join users u1 on u1.id_user=m.id_sender where u.id_user = '{$user}' group by c.id_conv order by m.time";
    $query_na = "SELECT u1.name FROM users u join messages m on u.id_user=m.id_user join conv c on m.id_conv=c.id_conv join users u1 on u1.id_user=m.id_sender where u.id_user = '{$user}' group by c.id_conv order by m.time";
    $query_su = "SELECT u1.surname FROM users u join messages m on u.id_user=m.id_user join conv c on m.id_conv=c.id_conv join users u1 on u1.id_user=m.id_sender where u.id_user = '{$user}' group by c.id_conv order by m.time";
    $query_ro = "SELECT r.name_role FROM users u join messages m on u.id_user=m.id_user join conv c on m.id_conv=c.id_conv join users u1 on u1.id_user=m.id_sender join roles r on r.id_role=u1.id_role where u.id_user = '{$user}' group by c.id_conv order by m.time";
